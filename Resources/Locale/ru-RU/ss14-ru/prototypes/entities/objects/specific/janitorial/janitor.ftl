...
ent-GoldenPlunger = золотой вантуз
    .desc = Вантуз с пластиковым клапаном, покрытым тонким слоем золота, который выдаётся в благодарность за многолетнюю службу. Все так же используется для прочистки канализации.
ent-RagItem = тряпка
    .desc = Предположительно, для устранения беспорядка.
ent-WireBrush = проволочная щетка
    .desc = Жесткая стальная проволочная щетка, идеально подходящая для соскабливания даже самых стойких пятен ржавчины.
